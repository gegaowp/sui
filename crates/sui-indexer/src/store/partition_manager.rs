...
impl From<EpochToCommit> for EpochPartitionData {
    fn from(epoch: EpochToCommit) -> Self {
        let last_epoch = epoch.last_epoch.map(|e| e.epoch).unwrap_or(0);
        let next_epoch = epoch.new_epoch.epoch;
        let last_epoch_start_cp = epoch
            .last_epoch
            .map(|e| e.start_checkpoint.sequence_number)
            .unwrap_or(0);
        let next_epoch_start_cp = epoch.new_epoch.start_checkpoint.sequence_number;
        Self {
            last_epoch,
            next_epoch,
            last_epoch_start_cp,
